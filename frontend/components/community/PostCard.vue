<template>
  <article class="bento-card group overflow-hidden cursor-pointer">
    <!-- Visual Header (Placeholder for Post Image) -->
    <div
      class="relative h-64 w-full overflow-hidden bg-slate-800 bg-center bg-cover"
      :style="coverUrl ? { backgroundImage: `url(${coverUrl})` } : {}"
    >
      <div class="absolute inset-0 bg-gradient-to-br from-indigo-500/25 to-purple-500/25 mix-blend-overlay"></div>
      ...
      <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
        <button 
          ...
          :class="isLiked ? 'bg-gradient-to-br from-rose-500 to-pink-600 text-white shadow-lg shadow-rose-500/30' : 'bg-white/10 hover:bg-white/20 text-white'"
        >
        ...
        <button 
          ...
          class="p-2 rounded-full backdrop-blur-md transition-all duration-300 active:scale-90"
          :class="isFavorited ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/30' : 'bg-white/10 hover:bg-white/20 text-white'"
        >
    ...
    <div class="p-5 space-y-3">
      <header class="flex items-start justify-between gap-4">
        <h3 class="text-lg font-bold text-white leading-snug group-hover:text-indigo-300 transition-colors line-clamp-2">
          {{ post.title }}
        </h3>
      </header>
      ...
      <div v-if="post.link_url" class="flex items-center gap-2 text-xs">
        <button
          class="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-indigo-500/10 text-indigo-200 border border-indigo-500/30 hover:bg-indigo-500/20 transition-colors"
      ...
      <div class="flex flex-wrap gap-2 pt-1">
        <span v-for="topic in post.topic_ids?.slice(0, 2) || []" :key="topic" class="text-[10px] font-medium px-2 py-0.5 rounded bg-white/5 text-slate-300 border border-white/5">
          #{{ topic }}
        </span>
      </div>

      <footer class="pt-3 mt-3 border-t border-white/5 flex items-center justify-between text-xs text-slate-500">
        <div class="flex items-center gap-2">
          <img 
            ...
            alt="Avatar"
          />
          <div v-else class="w-5 h-5 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-[10px] text-white font-bold">
